...
class SignalGenerator:
    def __init__(self):
        ...
        """
        try:
            # CORREÇÃO #1: Sincronizado com o subdomínio correto homologado no cluster
            supabase_url = os.getenv("SUPABASE_URL", "https://mandacarurn.com.br")
            supabase_key = os.getenv("SUPABASE_KEY")
            
            # CORREÇÃO #2: Impede injeção de chave nula na biblioteca do Supabase para evitar crash fatal
            if not supabase_key:
    ...
    def generate_signal(self, operation_type, quantity, price, symbol, status="new", stop_loss=None, take_profit=None):
        ...
        """
        try:
            # CORREÇÃO #3: Encapsulado com round(..., 4) para garantir conformidade com as regras de ticks da Binance
            ...
            # Executa a gravação na nuvem apenas se a instância do cliente estiver ativa e segura
            if self.supabase:
