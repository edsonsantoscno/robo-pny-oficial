...
class RiskManager:
    def __init__(self, banca_inicial, take_profit_meta_percent=0.5):
        ...
        if state:
            ...
            self.entry_price = state.get("entry_price") if state.get("entry_price") else None
            self.entry_quantity = state.get("entry_quantity") if state.get("entry_quantity") else None
    ...
    def clear_position(self):
        """Limpa a posição e o símbolo atual após a venda"""
        self.position_active = False
        self.current_symbol = None
        self.entry_price = None
        self.entry_quantity = None
    ...
    def atualizar_banca(self, valor): 
        ...
        self._persist() # Força a atualização do ganho no JSON para o dashboard ler
    ...
    def get_meta_diaria(self): 
        # Usa o valor configurado no config.py (ex: 2%)
    ...
    def check_stop_loss(self, price):
        if not self.position_active or not self.entry_price: return False, 0.0
    ...
    def _persist(self):
        # Alinhamento exato de assinatura para aceitar o dicionário ou argumentos do state_manager
        try:
            save_state(
                ...
                ganho_dia=self.get_ganho_atual(), # Garante a chave lida pelo app.py do dashboard
                current_symbol=self.current_symbol,
                banca_atual=self.banca_atual
            )
        except TypeError:
            # Fallback caso seu state_manager exija parâmetros posicionais estritos
            save_state(
