...
# CORREÇÃO #2: Caminho unificado correto na raiz do contêiner Docker conforme docker-compose
    ...
    with state_lock:
        if STATE_FILE.exists():
            try:
                ...
                print("⚠ Arquivo de estado corrompido ou ocupado. Ignorando ciclo...")
                return {}
        return {}
...
class TradingBotTrader:
    ...
    def run_once(self):
        try:
            ...
            if self.risk_manager.position_active:
                ...
                print(f"👀 Monitorando {symbol}: Preço ${price:.4f} | Lucro Atleta: {pct_atual:.2f}% (SL Alvo: -{sl_percent}% / TP Alvo: {tp_percent}%) | Sinal: {signal}")
            ...
            # 5. SCANNER DE MERCADO PARA NOVAS ENTRADAS
    ...
    def _execute_buy(self, symbol, price):
        """Dispara a ordem de compra e configura alvos OCO de segurança de rede"""
        calc = self.order_manager.calculate_quantity(symbol)
        if not (calc and self.order_manager.validate_order(symbol, "BUY", calc["quantity"])):
            return False
        ...
        # Projeta alvos matemáticos precisos para a Exchange
        ...
        print(f"🛡 [Binance OCO] Enviando travas nativas para a Exchange:")
        print(f" 🎯 Take Profit: {preco_take} | 🛑 Stop Loss: {preco_stop}")

        # Envia ordem OCO protegida para os servidores remotos da corretora
        try:
            ...
            print(f"⚠ Alerta OCO: Falha ao enviar OCO nativo na exchange (gerenciamento local ativo): {oco_err}")

        # Atualiza as chaves operacionais do estado local unificado
        ...
        # 1. PERSISTE SINAL DE HISTÓRICO NO BANCO (SUPABASE)
        ...
        # 2. CORREÇÃO #3: Exportação síncrona em lista para leitura unificada do Dashboard
        try:
            signal_payload = {
                "action": "BUY",
                "symbol": symbol,
            ...
            # Gerencia uma lista rotativa de sinais físicos na raiz para leitura do app.py
            with state_lock:
                existing_signals = []
                if SIGNALS_FILE.exists():
                    with open(SIGNALS_FILE, 'r', encoding='utf-8') as f:
                        try: existing_signals = json.load(f)
                        except: existing_signals = []
                
                existing_signals.append(signal_payload)
                with open(SIGNALS_FILE, 'w', encoding='utf-8') as f:
                    json.dump(existing_signals[-10:], f, indent=4, ensure_ascii=False)
            print("📡 Sinais integrados com sucesso na central física unificada do cluster.")
    ...
    def _execute_sell(self, symbol, price, reason):
        ...
        print(f"⏹ Executando saída de {symbol} por motivo: {reason}")
        
        # Limpa ordens em aberto na corretora para evitar preenchimentos falsos secundários
        try:
            ...
            print(f"⚠ Nota ao limpar ordens OCO: {c_err}")
...
if __name__ == "__main__":
    try:
        ...
        print("\n ⏹ Robô de trading desligado via terminal de forma segura.")
